...
unsigned Type::size() const
{
    ...
    if (_indirection > 0) return count * SIZEOF_PTR;

    else if (_specifier == INT) return count * SIZEOF_INT;

    else if (_specifier == CHAR) return count * SIZEOF_CHAR;
...
unsigned Type::alignment() const
{
    ...
    if (_indirection > 0) return ALIGNOF_PTR;

    else if (_specifier == CHAR) return ALIGNOF_CHAR;

    else if (_specifier == INT) return ALIGNOF_INT;
